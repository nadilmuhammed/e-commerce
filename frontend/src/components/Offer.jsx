...
const Offer = () => {
    const calculateTimeLeft = () =>{
        const difference = + new Date("2024-10-12T12:15:00") - +new Date();
        let timeleft = {}
        if(difference > 0) {
            timeleft = {
    ...
    const [timeLeft, settTimeLeft] = useState(calculateTimeLeft());

    useEffect(()=>{
        const timer = setTimeout(() => {
            settTimeLeft(calculateTimeLeft());
        }, 1000);
  ...
  return (
    <section className='max-padd-container bg-banner bg-center bg-cover h-[555px] p-8 rounded-xl my-8'>
        <div className='flex items-center flex-col gap-1 mt-40'>
            <h3 className='uppercase medium-20'>Sales Fever</h3>
            <h2 className='bold-44 upper\'>20% Off EveryThing</h2>
